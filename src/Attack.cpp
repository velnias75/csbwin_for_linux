...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      ...
      || vorpalOrDisrupt  )
  {
    ...
    if (throwingDistance != 0)  /////goto tag00faa2;
    {
      ...
      D4W = sw((ranResult=STRandom(32)) + pmtDesc->uByte8[0] + levelDifficulty);
      if (traceID!=NULL)
      {
        fprintf(GETFILE(TraceFile),"%sD4W = (i26->uByte8[0]=%d) + (levelDifficulty=%d) + (random(32)=%d) = %d\n",
                     traceID,pmtDesc->uByte8[0],levelDifficulty,ranResult,D4W);
      };
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if (damage < 0)
  {
    ...
    TextOutToScreen(D5W, 100, 4, 0, (const pnt)A2, true);
  }
  else
  {
    if (damage > 40)
    ...
    else
    {
      ...
      if (D0W == 0)
      {
        A2 = (i8 *)GetBasicGraphicAddress(14);
        ShrinkBLT((i8*)A2,
                  A3 = GetDerivedGraphicAddress(D7W),
                  96,
                  45,
                  D5W,
                  37,
    ...
    TextOutToScreen(D5W, 100, 4, 0, &b_6[D7W]);
  };
  VBLDelay(_1_,15);
  STShowCursor(HC1);
  RETURN;
...
//
  ...
  PrintLines(0, d.Byte1830); // Linefeed
  PrintLines(15, (const pnt)"THE END.");
  PrintLines(0, d.Byte1830); // Linefeed
  VBLDelay(_19_,600);
