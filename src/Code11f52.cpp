...
//*********************************************************
//   TAG00135c
void GetIconGraphic(OBJ_NAME_INDEX iconNum,pnt dest)
{
  dReg D0, D1, D6, D7;
...
//
      ...
      D0W = (i16)(D0W - (D4W+1));
      MemMove((pnt)prnA3, (pnt)prnA3+2, 2*D0W);
      *prnA3 = newObj;
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ClearMemory((pnt)DB4A3->hitPoints6, 8);
      ...
      if (objectDB == dbACTUATOR)
      {
        ...
        if (actuatorType == 47)
        {
          missileEncounterDSA = obj;
          {
            ...
            timer.timerUByte9(0);//timerFunction
            timer.timerUByte8(0);//timerPosition
            timer.timerUByte7((ui8)missileEncounterFilterLocr.y);
            timer.timerUByte6((ui8)missileEncounterFilterLocr.x);
            timer.timerTime   = missileEncounterFilterLocr.l << 24;
            pDSAparameters[0] = 7;
            ...
            ProcessDSAFilter(missileEncounterDSA, &timer, missileEncounterFilterLocr, NULL, &dsaVars);
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ClearMemory((pnt)whoLeavesCell,4);
  ...
  if (newX >= 0)
  {
    ...
    */
    if (distanceTraveled == 1)
    {
      ...
      ClearMemory((pnt)whoLeavesNewCell,4);
      //D0W = occupiedPosition[leadPosLeft];
...
//Loop:
  for (;numCellsToTry>0; numCellsToTry--)
  {
    ...
    if (numCellsToTry > 1)
    {
      ...
      missileY = newY;
      //oldY |= (newY << 8);
      MemMove((pnt)&whoLeavesNewCell, (pnt)&whoLeavesCell, 4);
      //goto Loop;
    };
  };
  return 0;
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if (pDB3->w4_b11())
  {
    PerformLocalAction(pDB3->w6_4_15(), mapX, mapY, P5);
  }
  else
  {
...
tag015560:
  ...
  if (objNI_2 != objNID5)
  {
    d.Word16608 = 1;
    STHideCursor(HC10);
    GetIconGraphic(objNI_2, d.Pointer16572);
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  {
    MemMove((pnt)&d.PartyFootprints[D7W+1],(pnt)&d.PartyFootprints[D7W],2*D6W);
    MemMove(&d.Byte13220[D7W+1],&d.Byte13220[D7W],D6W);
  };
