...
//*********************************************************
//   TAG01f990
void DisplayHeroName(i16 P1,i16 P2,i16 P3,pnt heroName)
{
  aReg A2, A3;
  i8 b_50[50];
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if (P1)
  {
    A2 = A3;
    A3 += 560;
    ReadAndExpandGraphic(6, A2, 0, 0);
    if (d.Word11694)
    {
      LOCAL_14 = A3;
      A3 += 1376;
      ReadAndExpandGraphic(208, LOCAL_14, 0, 0);
    };
  };
  ReadAndExpandGraphic(0x8005, A3, 0, 0);
  LOCAL_10 = A3;
  closeGraphicsFile();
  if (d.DynamicPaletteSwitching)
  {
    MemMove((pnt)&d.Palette11978, (pnt)&LOCAL_78, 32);
    for (D6W=0; D6W<=7; D6W++)
    {
      wvbl(_2_);
...
//
    ...
    wvbl(_3_);
    MemMove((pnt)&LOCAL_78, (pnt)&d.Palette11978, 32);
  }
  else
  {
  ...
  if (P1)
  {
    if (d.Word11694)
    {
      ...
      for (D7W=0; D7W<d.NumCharacter; D7W++)
      {
        ...
        pcA0 = &d.CH16482[D7W];
        LOCAL_6 = pcA0;
        BLT2Screen(LOCAL_14, (RectPos *)d.Word132, 32, 10);
        BLT2Screen(LOCAL_6->portrait, (RectPos *)d.Word140, 16, 1);
        ...
        for (D5W=0; D5W<=3; D5W++)
        {
          ...
          StrCpy(LOCAL_130, d.Pointer16770[D4W]);
          StrCat(LOCAL_130, " ");
          StrCat(LOCAL_130, d.Pointer16596[D5W]);
          DisplayHeroName(105, D6W=sw(D6W+8), 13, LOCAL_130);
//
        };
        d.Word132[3] += 48;
        d.Word132[2] += 48;
...
tag01fd10:
    ...
    BLT2Screen(A2, (RectPos *)d.Word108, 40, -1);
    fillMemory((i16 *)&LOCAL_110, 16, 2, 2);
    MemMove((pnt)&LOCAL_110, (pnt)&LOCAL_46, 32);
    ForceScreenDraw();
...
//
      ...
      { //No. The player pressed 'Restart This Game'
        ...
        MemMove((pnt)&d.Palette552[0], (pnt)&d.Palette11946, 32);
