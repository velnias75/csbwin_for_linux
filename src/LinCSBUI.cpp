	...
	switch(doInitializeSounds) {
		...
		case SOUND_IS_ESD: {
			...
			//	csl_pcm_write(AUDIO, size, audio);
				const ssize_t result = write(AUDIO[AUDIOPOS],audio+WAV_OFFSET,size);
				if (result < 0) {
					g_warning("Broken pipe to sound device. Trying to reconnect.");
					UI_Initialize_sounds();
					goto FAILED_UTTERLY;
...
//SDL_AudioSpec *SDL_LoadWAV(const char *file, SDL_AudioSpec *spec, Uint8 **audio_buf, Uint32 *audio_len);
  return LIN_PlaySound((char*)wave/* + 58*/,((SNDHEAD*)wave)->Size - WAV_OFFSET+sizeof(i32), SDL_MIX_MAXVOLUME);
}
enum {
    		    enum_directory=42,
