...
{ // volume is a divisor
  ...
  if(f)
  {
    for (pnt kk=pGraphic; kk<pN+1; kk+=8)
    {
...
//*********************************************************
//   TAG001d16
void StartSound(pnt SoundBytes,
                i32 /*timerData*/,
                i32 highVolume)
{
  i32 size, volume;
  ...
  Wave = SoundDecode(SoundBytes+2, size, volume);
...
void PlayCustomSound(i32 soundNum, i32 volume, i32 /*flags*/)
{
  ...
  if (soundNum < 0)
  {
    ...
    StartSound((pnt)GetBasicGraphicAddress(pSound->word0|0x8000),// Start Sound
               (UI8)(pSound->byte3),
               volume);
  }
  else
