...
void GTrace_4(char *s, i32 a, i32 b, i32 c, i32 d)
{
  if (GraphicTraceActive)
  {
    fprintf(GETFILE(GraphicTraceFile), s, a, b, c, d);
  };
}

void GTrace_6(char *s, i32 a, i32 b, i32 c, i32 d, i32 e, i32 f)
{
  if (GraphicTraceActive)
  {
    fprintf(GETFILE(GraphicTraceFile), s, a, b, c, d, e, f);
  };
}

void GTrace_8(char *s, i32 a, i32 b, i32 c, i32 d, i32 e, i32 f, i32 g, i32 h)
{
  if (GraphicTraceActive)
  {
    fprintf(GETFILE(GraphicTraceFile), s, a, b, c, d, e, f, g, h);
  };
}

void GTrace_12(char *s, i32 a, i32 b, i32 c, i32 d, i32 e, i32 f, i32 g, i32 h, i32 i, i32 j, i32 k, i32 l)
{
  if (GraphicTraceActive)
  {
    fprintf(GETFILE(GraphicTraceFile), s, a, b, c, d, e, f, g, h, i, j, k, l);
  };
}
...
void InterpretError(const i8 *msg)
{
  UI_MessageBox(msg, "Viewport Display Error", MESSAGE_OK);
}

void InterpretError(const i8 *msg, int param)
{
  i8 m[1000];
  sprintf(m, msg, param);
  InterpretError(m);
...
pnt StdBitmapPointers(i32 n)
{
  ...
  case BMP_StdHoleInWallBitmap:                return (pnt)GetBasicGraphicAddress(41);
  };
               ...
               )
{
  ...
  for (;;)
  {
    ...
    case StdDrawSeeThruDoorEdge:
               MemMove(d.pDoorBitmaps[1], (i8 *)tempBitmap, 1968);
               TAG0088b2(GetBasicGraphicAddress(41),
                  ...
                  16,
                  9);
               BltShapeToViewport((i8 *)tempBitmap, &d.DoorTrackTopRect[6]);
                break;
    case StdDrawDoor:
    ...
    case StdBltShapeToViewport:
            pRectangle = StdRectanglePointers(pop());
            pBitmap = StdBitmapPointers(pop());
            BltShapeToViewport( pBitmap, pRectangle);
            break;
    case StdMirrorShapeToViewport: 
      pRectangle = StdRectanglePointers(pop());
      pBitmap = StdBitmapPointers(pop());
      MemMove(pBitmap,(i8 *)tempBitmap,pRectangle->b.uByte4*pRectangle->b.uByte5);
      MirrorShapeBltToViewport((i8 *)tempBitmap, pRectangle);
      break;
    case Literal:
      push(*(P++));
      break;
    ...
    case StdBltToDerivedGraphic:
      {
        ...
        dstBitmap = GetDerivedGraphicAddress(pop());
        srcBitmap = StdBitmapPointers(pop());
        TAG0088b2(
          (ui8 *)srcBitmap,
          (ui8 *)dstBitmap,
          pDstRect,
    ...
    case StdBltFromDerivedGraphic:
      {
        ...
        srcX = pop();
        pDstRect = StdRectanglePointers(pop());
        dstBitmap = StdBitmapPointers(pop());
        srcBitmap = GetDerivedGraphicAddress(pop());
        TAG0088b2(
          (ui8 *)srcBitmap,
          (ui8 *)dstBitmap,
          pDstRect,
...
BACKGROUND_MASK *BACKGROUND_LIB::CreateNewMask(ui32 id, ui32 num, BACKGROUND_MASK *pMask)
{ // size in bytes including the header.
  ...
  if (pMaskIndex[0] <= num)
  {
    ...
    n = num - pMaskIndex[0] + 1; // # index entries we need to add.
    ...
    memmove(pbg + 4 * num + 8,
            ...
            backgroundGraphics[idx].size - 4 * num - 8);
  ...
  pMaskIndex[num+1] = backgroundGraphics[idx].size;
  ...
  backgroundGraphics[idx].data 
        = (ui8 *)UI_realloc(backgroundGraphics[idx].data,
                            ...
                            MALLOC101);
  pMaskIndex = (ui32 *)backgroundGraphics[idx].data;
  memcpy (backgroundGraphics[idx].data + pMaskIndex[num+1], pMask, sizeof (*pMask));//gsize);
  return (BACKGROUND_MASK *)(backgroundGraphics[idx].data + pMaskIndex[num+1]);
}

AFFINEMASK *BACKGROUND_LIB::CreateNewAffineMask(ui32 id, ui32 num, AFFINEMASK *pMask)
{ // size in bytes including the header.
  static bool errorMsg = false;
  ui8 *pbg;
  ui32 actualSize, n, i, gsize;
  ...
  backgroundGraphics[idx].data 
        = (ui8 *)UI_realloc(backgroundGraphics[idx].data,
                            ...
                            MALLOC101);
  pMaskIndex = (ui32 *)backgroundGraphics[idx].data;
  memcpy (backgroundGraphics[idx].data + pMaskIndex[num+1], pMask, gsize);
  return (AFFINEMASK *)(backgroundGraphics[idx].data + pMaskIndex[num+1]);
}
...
//#endif
  ...
  fillWithByte((pnt)&d.ViewportObjectButtons[0],6,ui8(0xff), 4);
  ...
  { //Mirror Floor
    ...
    D4W = 1;//d.BigMemory;
    d.pWallBitmaps[4] = d.Pointer2214;
    d.pWallBitmaps[3] = d.Pointer2218;
    d.pWallBitmaps[2]  = d.Pointer2222;
    d.pWallBitmaps[1]  = d.Pointer2226;
    d.pWallBitmaps[0]  = d.Pointer2230;
  }
  else
  { //Mirror Ceiling
...
//};
  ...
  if (!skipDrawing)
  {
    ...
    {
      NotImplemented();//DrawCellF1R1(facing,&roomData[RF1R1]);
    };
  };
  ...
  if ((D0W=D4W)!=0)
  {
    d.pWallBitmaps[4] = d.Pointer2234;
    d.pWallBitmaps[3] = d.Pointer2238;
    d.pWallBitmaps[2]  = d.Pointer2242;
    d.pWallBitmaps[1]  = d.Pointer2246;
    d.pWallBitmaps[0]  = d.Pointer2250;
  };
...
void DisplayFoodWater(void)
{
  ...
  RepackChest();
  BLT2Viewport((pnt)GetBasicGraphicAddress(20), &d.wRectPos926, 72, 8);//Food/water box
  BLT2Viewport((pnt)GetBasicGraphicAddress(30), &d.wRectPos950, 24, 12);//"Food"
  BLT2Viewport((pnt)GetBasicGraphicAddress(31), &d.wRectPos958, 24, 12);//"Water"
  if (pcA3->poisonCount != 0)
  {
    BLT2Viewport((pnt)GetBasicGraphicAddress(32), &d.wRectPos966, 48, 12);
  };
