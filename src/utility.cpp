...
struct STRUCT148
{
  i8   *((*malloc)(i32 size));                 //0  //148
  void  (*free)(i8 *address);                  //4  //144
  void  (*seek)(i32 handle, i32 address);      //8  //140
  void  (*read)(i32 handle, i32 size, i8 *buf);//12 //138
  i32   handle;                                //16 //132      
  i32   i_20;                                  //20 //128
  i8   *p_24;                                  //24 //124
...
i8   *MallocMemory(i32 size);//TAG0000b4
void  FreeMemory(i8 *address);//TAG0000ec
void  FileSeek(i32 handle, i32 offset);//TAG00010e
void  ReadFile(i32 handle, i32 size, i8 *buf);//  TAG000144
...
char *MALLOC(i32 size)
{
  if (size == -1) return (char *)1000000;
  return (char *)UI_malloc(size, MALLOC067);
}

void MFREE(char *address)
{
  UI_free (address);
}
...
//*********************************************************
//  TAG000144
void ReadFile(i32 handle, i32 size, i8 * buffer)
{
  dReg D0;
...
//
//*********************************************************
static i8 *TAG0004fe(char *dest, const char *src)
{
  const i8 *A2;
  i8 *A3, *p_4;
...
//
    ...
    s148->read(s148->handle, i_14, (i8 *)pSegDesc);
...
//
    ...
    s148->read(s148->handle, 40, b_114);
    i_14 =  LE32(longGear(b_114+4))
          + LE32(longGear(b_114))
          + LE32(longGear(b_114+20))
          + 12;
    p_42 = s148->malloc(i_14);
    s148->p_36 = p_42;
    p_30 = p_42;
    ...
    i_14 = D0L - 40;
    s148->read(s148->handle, i_14, p_42);
...
//
  ...
  if (pSegDesc != NULL) s148->free((i8 *)pSegDesc);
 
  if (p_50 != NULL) s148->free(p_50);
